...
# TODO:对半分，其中一半保持原样，另一半用作GhostModule
def _split_channels(origin_channels, num_groups):
    channels = origin_channels // 2
    split_channels = [channels // num_groups for _ in range(num_groups)]
    split_channels[0] += channels - sum(split_channels)
    split_channels += [channels] # 这一半留用冗余特征图
...
class DynamicSplitConvolution(nn.Module):
    ...
    def __init__(self, channels, stride, num_branch, num_groups, num_kernels, with_cp=False):
        ...
        ] + [nn.Sequential( # cheapOperation
                nn.Conv2d(self.split_channels[-1], self.split_channels[-1], kernel_size=3, stride=1, padding=1, groups=self.split_channels[-1], bias=False),
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            values = avgpool_x.view(avgpool_x.size(0), -1)  # 将特征图展平为一维向量
            sorted_indices = torch.argsort(values, dim=1, descending=False)  # 对数值进行升序排序，返回排序后的索引

            # 根据排序后的索引重新排列输入 x
            x = torch.gather(x, 1, sorted_indices.unsqueeze(-1).unsqueeze(-1).expand(-1, x.size(1), x.size(2),
                                                                                            x.size(3)))

            x_split = torch.split(x, self.split_channels, dim=1) # 把avgpool不行的通道放到后边
            x = [conv(t) for conv, t in zip(self.conv[:-1], x_split[:-1])] # 对其中一半不做操作,相当于dropout掉
            x = torch.cat(x, dim=1)

            x_ghost = self.conv[-1](x) # 将DKA输出的结果，作为冗余特征图的输入
            x = torch.cat([x, x_ghost], dim=1) # 连接起来作为整体输出
