...
class ConditionalChannelWeighting(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_resolution_weighting = CrossResolutionWeighting( # TODO：5另一些可替换的玩意
            branch_channels,
            ratio=reduce_ratio,
            conv_cfg=conv_cfg,
            norm_cfg=norm_cfg)
        ...
        self.spatial_weighting = nn.ModuleList([
            SpatialWeighting(channels=channel, ratio=4) # TODO：7通通替换
...
class InceptionDWConv2d(nn.Module):
    def __init__(self, in_channels, square_kernel_size=3, band_kernel_size=11, branch_ratio=1 / 4):
        super().__init__()
...
class ShuffleUnit(nn.Module):
    def __init__(self,
        ...
        self.branch2 = nn.Sequential(
            InceptionDWConv2d(in_channels if (self.stride > 1) else branch_features),
            ConvModule(
...
class LiteHRModule(nn.Module):
    ...
    def _make_weighting_blocks(self, num_blocks, reduce_ratio, stride=1):
        ...
        for i in range(num_blocks):
            layers.append(
                ConditionalChannelWeighting( # TODO：3，在这里有它的条件通道加权层
                    self.in_channels,
                    stride=stride,
